...
export function Filter({ onClick }: { onClick: (f: string | undefined) => void }) {
	const animRef = useRef(null);
	const isInView = useInView(animRef, { once: true });
	const [activeFilter, setActiveFilter] = useState<undefined | string>(undefined);

	const handleFilterClick = (filter: undefined | string) => {
		onClick(filter);
